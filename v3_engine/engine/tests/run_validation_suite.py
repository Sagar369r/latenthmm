...
class TestExecutionAndSurveillance:
    ...
    def test_triple_gate_all_combinations(self):
        ...
        # Momentum gate fails
        r = gate.evaluate(p_mean_rev=0.10, p_trend=0.80, mvt=0.5, qt=1.8)
        assert not r["all_pass"] and r["direction"] == 0

        # Volume gate fails
        r = gate.evaluate(p_mean_rev=0.10, p_trend=0.80, mvt=2.0, qt=1.1)
        assert not r["all_pass"] and r["direction"] == 0

        # Boundary — exactly at threshold must NOT pass (strict >)
        r = gate.evaluate(p_mean_rev=0.10, p_trend=0.65, mvt=2.0, qt=1.8)
        assert not r["regime_gate"], "p_trend == 0.65 should NOT pass (strict >)"

        r = gate.evaluate(p_mean_rev=0.10, p_trend=0.80, mvt=1.0, qt=1.8)
        assert not r["momentum_gate"], "|mvt| == 1.0 should NOT pass (strict >)"

        r = gate.evaluate(p_mean_rev=0.10, p_trend=0.80, mvt=2.0, qt=1.3)
