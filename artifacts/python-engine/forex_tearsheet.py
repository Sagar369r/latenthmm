...
ATR_SL_MULT      = 3.0    # stop-loss  (was 1.5 in the SPY test)
ATR_TP_MULT      = 5.0    # take-profit (was 3.0 in the SPY test)
...
# Walk-forward parameters (same proven window that avoids HMM collapse)
WF_TRAIN_BARS = 504   # 2 years of daily trading days
WF_OOS_BARS   = 126   # 6 months of daily trading days
...
def phase1_build_data() -> dict:
    ...
    print(f"  Fetching {TICKER} daily bars {DATA_START} → {DATA_END} …")
    raw = yf.download(
    ...
    # Build a clean bars_df (OHLCV) — forex Volume is unreliable, set to NaN
    bars_df          = raw[["Open", "High", "Low", "Close"]].copy()
    bars_df.columns  = ["open", "high", "low", "close"]
    bars_df["volume"] = np.nan   # volume not available for EURUSD

    print(f"  {len(bars_df)} daily bars  ({bars_df.index[0].date()} → {bars_df.index[-1].date()})")

    # ── Build range-ratio proxy for qt (replaces dollar-volume) ────────────
    #   range_ratio = (H-L) / EMA20(H-L)  ≡ qt for equities
    range_series = (bars_df["high"] - bars_df["low"])
    range_ema    = range_series.ewm(span=20, min_periods=5).mean()
    range_ratio  = (range_series / range_ema.clip(lower=1e-10)).clip(lower=0.01, upper=10.0)

    # ── 6D feature tensor ──────────────────────────────────────────────────
    print("  Computing 6D feature tensor …")
    # compute_feature_tensor expects volume; use range as proxy
    bars_for_feat           = bars_df.copy()
    bars_for_feat["volume"] = range_series   # use range as surrogate volume
    features = compute_feature_tensor(bars_for_feat)

    # Overwrite qt with our range-ratio (the built-in qt uses dollar-volume)
    features["qt"] = range_ratio
...
def phase3_generate_signals(d: dict, regime_proba: np.ndarray) -> list[dict]:
    ...
    q_t_arr   = range_ratio.fillna(1.0).values
    ...
    for t in range(1, T):
        ...
        gr = gate.evaluate(p_trend, mv_t, q_t)
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 4 — Event-driven backtest (pip-based friction, forex stops)
# ─────────────────────────────────────────────────────────────────────────────
def run_backtest(
    ...
    """
    Bar-by-bar forex simulation.

    Entry  : close of signal bar t  (+ half-spread for entry cost)
    SL/TP  : checked against bar t+1 H/L onwards
    Trail  : breakeven at +2 ATR; trail at 1.5 ATR behind peak at +3 ATR
    Spread : round-trip cost = spread_pips × PIP_SIZE applied at entry
    ...
    for t in range(len(bars_df)):
        ...
        if position is not None and not position.get("skip_exit_this_bar", False):
            ...
            # Trailing stop logic (forex-wider parameters)
            if atr > 0:
                pnl_atr = (price - entry_p) * d_dir / atr
                if pnl_atr >= ATR_TRAIL_TRIGGER and not position["trail_activated"]:
                    position["stop_loss"] = entry_p
                    position["trail_activated"] = True
                    sl = entry_p
                if pnl_atr >= (ATR_TRAIL_TRIGGER + 1.0):
                    if d_dir == 1:
                        position["highest_fav"] = max(position["highest_fav"], price)
                        new_sl = position["highest_fav"] - atr * ATR_TRAIL_DIST
                        position["stop_loss"] = max(sl, new_sl)
                        sl = position["stop_loss"]
                    else:
                        position["lowest_fav"] = min(position["lowest_fav"], price)
                        new_sl = position["lowest_fav"] + atr * ATR_TRAIL_DIST
            ...
            if exit_price is not None:
                ...
                exit_spread = spread_pips * PIP_SIZE * units * 0.5
        ...
        # ── Check for EOD flush ──────────────────────────────────────────
        # (forex: flush only if trade is very old — 60+ bars = ~2 months)
        if position is not None:
            ...
            if bars_held >= 60:
                ...
                entry_p = position["entry_price"]
                units   = position["units"]
                raw_pnl = (price - entry_p) * d_dir * units
                exit_spread = spread_pips * PIP_SIZE * units * 0.5
        ...
        if position is None and t in signal_map:
            ...
            if atr_s > 0 and current_equity > 0:
                ...
                entry_spread = spread_pips * PIP_SIZE * units * 0.5
...
def buy_and_hold_benchmark(bars_df: pd.DataFrame) -> dict:
    """EUR/USD buy-and-hold from OOS_START."""
...
def phase6_validation(oos_returns: np.ndarray, n_trades: int) -> dict:
    ...
    # ── 6.1  Walk-Forward CV ───────────────────────────────────────────────
    # simulate_fn(train_start, train_end, test_start, test_end) → (is, oos)
    def wf_fn(ts, te, ss, se):
        return oos_returns[ts:te], oos_returns[ss:se]

    try:
        wf_res = _wfcv(
            wf_fn, T,
            train_bars=WF_TRAIN_BARS,
            test_bars=WF_OOS_BARS,
            min_bars=252 * 5,
        )
        wf = {
            "passes":       wf_res.passes,
            "oos_sharpe":   wf_res.mean_oos_sharpe,
            "is_oos_ratio": wf_res.overfit_ratio if wf_res.mean_oos_sharpe != 0 else float("inf"),
            "n_windows":    len(wf_res.windows),
        }
    except Exception as e:
        wf = {"passes": False, "oos_sharpe": 0.0, "is_oos_ratio": 0.0, "n_windows": 0,
              "error": str(e)}

    # ── 6.2  Monte Carlo Permutation ──────────────────────────────────────
    # simulate_fn(permuted_returns) → oos_sharpe  (OOS = last 30 %)
    def mc_fn(shuffled):
    ...
    _vrow("6.1 Walk-Forward CV",
          f"OOS SR={wf['oos_sharpe']:.2f}  "
          f"IS/OOS={wf['is_oos_ratio']:.2f}  "
...
def print_tearsheet(
    ...
    print()
    _sep("LATENT DIFFUSION-HMM v3.0  —  EUR/USD FOREX TEAR SHEET")
    print(f"  Ticker     : {TICKER}  (daily bars, 2016–2024)")
    print(f"  WF-train   : {WF_TRAIN_BARS}-bar rolling window  |  OOS block: {WF_OOS_BARS} bars")
    print(f"  Stops      : SL={ATR_SL_MULT}×ATR  TP={ATR_TP_MULT}×ATR  "
          f"Trail trigger={ATR_TRAIL_TRIGGER}×ATR")
    ...
    print(f"  {'Metric':<24}  {'Strategy':>12}  {'B&H EUR/USD':>12}")
    print(f"  {'─'*22}  {'─'*12}  {'─'*12}")
    ...
    print(f"  Exit — Time (60b) : {m['time_count']}")
    ...
    if n_tr == 0:
        ...
        print("  → 6D features do not hold predictive edge for EUR/USD daily bars.")
...
def main() -> None:
    wall_t0 = time.time()
    print()
    _sep("LATENT DIFFUSION-HMM  —  EUR/USD FOREX WALK-FORWARD TEST")
    print(f"  Started  : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Protocol : Make or Break  |  "
          f"SL={ATR_SL_MULT}×ATR  TP={ATR_TP_MULT}×ATR  "
          f"Spread scenarios: 0/1.0/1.5/2.5 pips")
    ...
    # ── Phase 4: backtests under all pip scenarios ─────────────────────────
    print()
    _sep("Phase 4 — Event-Driven Backtest (4 pip-spread scenarios)")
    pip_results: dict[str, dict] = {}
    pip_metrics: dict[str, dict] = {}

    for label, pips in PIP_SCENARIOS:
        bt = run_backtest(bars_df, signals, equity=INITIAL_EQUITY, spread_pips=pips)
    ...
    baseline_metrics = pip_metrics["0 pip  (gross)"]
    baseline_bt      = pip_results["0 pip  (gross)"]
    ...
    # ── Save JSON ──────────────────────────────────────────────────────────
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "forex_results.json")
